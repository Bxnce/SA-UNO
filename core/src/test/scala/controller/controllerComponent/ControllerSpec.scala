...
class ControllerSpec extends AnyWordSpec {
  "Controller" should {
    ...
    "have a method take(), that calls the take() function in game" in {
      c.game.currentstate shouldBe (UnoState.between21State)
      c.next() //p1s
      c.take()
    ...
    "have a method place() that places a Card from a player to the midStack" in {
      ...
      c.game.currentstate shouldBe (UnoState.between12State)
      c.place(0)
      c.game.ERROR shouldBe (-1)
    }
    "have a method next()" in {
      c.game.currentstate shouldEqual (UnoState.between12State)
      c.next() //p2s
      c.game.currentstate shouldEqual (UnoState.player2State)
    }

    var game1 = new Game("p1", "p2", UnoState.between21State)
    ...
    "have a method undo() that undos the last Command" in {
      ...
      c1.next()
      c1.game.currentstate shouldEqual (UnoState.player1State)
      c1.undo()
      c1.game.currentstate shouldEqual (UnoState.between21State)
    ...
    var game2 = new Game("p1", "p2", UnoState.between21State)
    ...
    "have a method redo() that redos the last step that happend" in {
      c2.next()
      c2.game.currentstate shouldEqual (UnoState.player1State)
      c2.undo()
      c2.game.currentstate shouldEqual (UnoState.between21State)
      c2.redo()
      c2.game.currentstate shouldEqual (UnoState.player1State)
    ...
    "have a method newG(String,String) that creates a new Game" in {
      ...
      c.game.midCard.karten.size shouldBe (1)
      c.game.currentstate shouldEqual (UnoState.between21State)
    }
